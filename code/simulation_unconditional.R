...
Biweekly_stratification_select_1 <- function(Hazard_Periods){ #time stratified approach for 2 week periods
...
Month_stratification_select_1 <- function(Hazard_Periods){
...
FourWk_aka28day_stratification_select_1 <- function(Hazard_Periods){ #time stratified approach for 28 day periods for warm months
...
Case_Crossovers_Sample <- function(Params_for_Simulated_Year){
  ...
  ## Monthly Pre-filtering
  filtered_days_month <- CC_casedays %>%
    filter(date <= get_end_date(CC_casedays, "month"))

  month_control_days <- Month_stratification(filtered_days_month)

  month_control_days_sample <- month_control_days %>%
    group_by(Participant) %>%
    slice_sample(n=1) %>%
    ungroup()
  ...
  ## Conditional Setup
  Simulation_df_MonthStrat <- filtered_days_month %>%
    bind_rows(., month_control_days) %>%
    left_join(., CC_Exposures, by = "date") %>% arrange(Participant)
  ...
  #clogit regression - no adjustment
    ...
    CCOResults_monthstrat_cond <- broom::tidy(mod.clogit.month, conf.int = TRUE) %>%
      filter(term == "x") %>%
      mutate(Analysis = "CCO_Month") %>%
      add_column(Simulated_RR = Simulated_RR)

  ## Unconditional  Setup
  month_control_days_unc <- month_control_days_sample%>%
    left_join(., CC_Exposures, by = "date")  %>%
    rename(Control_Exposure = x,
           Control_Date = date,
           Control = Case)

  Simulation_df_MonthStrat_unc <- filtered_days_month %>%
    left_join(., CC_Exposures, by = "date") %>%
    rename(Case_Exposure = x,
           Case_Date = date) %>%
    left_join(month_control_days_unc,by = 'Participant') %>%
    mutate(Exp_Diff = Case_Exposure-Control_Exposure)

  unc_month_model <- glm(Case ~ 0+Exp_Diff,family=binomial,data=Simulation_df_MonthStrat_unc)

  CCOResults_monthstrat <- broom::tidy(unc_month_model, conf.int = TRUE) %>%
    mutate(Analysis = "CCO_Month") %>%
    add_column(Simulated_RR = Simulated_RR)
  ...
  ## 28day Pre-filtering
  filtered_days_28 <- CC_casedays %>%
    filter(date <= get_end_date(CC_casedays, "28_day"))

  d28_control_days <- FourWk_aka28day_stratification(filtered_days_28)

  d28_control_days_sample <- d28_control_days %>%
    group_by(Participant) %>%
    slice_sample(n=1) %>%
    ungroup()

  ## Conditional
  Simulation_df_28dayStrat <- filtered_days_28 %>%
    bind_rows(d28_control_days) %>%
    left_join(., CC_Exposures, by = "date")
  ...
  CCOResults_28daystrat_cond <- broom::tidy(mod.clogit.fourwk, conf.int = TRUE) %>%
    filter(term == "x") %>%
    mutate(Analysis = "CCO_28day") %>%
    add_column(Simulated_RR = Simulated_RR)

  ## Unconditional
  d28_control_days_unc <- d28_control_days_sample%>%
    left_join(., CC_Exposures, by = "date")  %>%
    rename(Control_Exposure = x,
           Control_Date = date,
           Control = Case)

  Simulation_df_d28Strat_unc <- filtered_days_28 %>%
    left_join(., CC_Exposures, by = "date") %>%
    rename(Case_Exposure = x,
           Case_Date = date) %>%
    left_join(d28_control_days_unc,by = 'Participant') %>%
    mutate(Exp_Diff = Case_Exposure-Control_Exposure)

  unc_d28_model <- glm(Case ~ 0+Exp_Diff,family=binomial,data=Simulation_df_d28Strat_unc)

  CCOResults_28daystrat <- broom::tidy(unc_d28_model, conf.int = TRUE) %>%
    mutate(Analysis = "CCO_28day") %>%
    add_column(Simulated_RR = Simulated_RR)
  ...
  ## 2Week Pre-filtering
  filtered_days_2w <- CC_casedays %>%
    filter(date <= get_end_date(CC_casedays, "2_week"))

  W2_control_days <- Biweekly_stratification(filtered_days_2w)


  W2_control_days_sample <- W2_control_days %>%
    group_by(Participant) %>%
    slice_sample(n=1) %>%
    ungroup()
  ...
  ## Conditional
  Simulation_df_2WeekStrat <- filtered_days_2w %>%
    bind_rows(W2_control_days) %>%
    left_join(CC_Exposures, by = "date")
  ...
  CCOResults_biweekstrat_cond <- broom::tidy(mod.clogit.2wk, conf.int = TRUE) %>%
    mutate(Analysis = "CCO_2week") %>%
    add_column(Simulated_RR = Simulated_RR)
  ...
  W2_control_days_unc <- W2_control_days_sample %>%
    left_join(., CC_Exposures, by = "date")  %>%
    rename(Control_Exposure = x,
           Control_Date = date,
           Control = Case)
  ...
  Simulation_df_W2Strat_unc <- filtered_days_2w %>%
    left_join(., CC_Exposures, by = "date") %>%
    rename(Case_Exposure = x,
           Case_Date = date) %>%
    left_join(W2_control_days_unc,by = 'Participant') %>%
    mutate(Exp_Diff = Case_Exposure-Control_Exposure)


  unc_W2_model <- glm(Case ~ 0+Exp_Diff,family=binomial,data=Simulation_df_W2Strat_unc)

  CCOResults_biweekstrat <- broom::tidy(unc_W2_model, conf.int = TRUE) %>%
    mutate(Analysis = "CCO_2week") %>%
    add_column(Simulated_RR = Simulated_RR)


  RegressionResults <- bind_rows(CCOResults_monthstrat,
                                 CCOResults_28daystrat,
                                 CCOResults_biweekstrat)

  RegressionResults_conditional <- bind_rows(CCOResults_monthstrat_cond,
                                             CCOResults_28daystrat_cond,
                                             CCOResults_biweekstrat_cond)

  return(list(unconditional = RegressionResults, conditional = RegressionResults_conditional))
}


Check_Control_DF <- function(Params_for_Simulated_Year){
  ...
  return(list(unconditional_2w = Simulation_df_W2Strat_unc, conditional_2w = Simulation_df_2WeekStrat,
              unconditional_4w = Simulation_df_d28Strat_unc, conditional_4w = Simulation_df_28dayStrat,
              unconditional_m = Simulation_df_MonthStrat_unc,conditional_m = Simulation_df_MonthStrat))
}
...
bias_comparison_visualization <- function(conditional_results,unconditional_results){
  ...
  bias_comp_plot <- ggplot(overall_results) +
    geom_boxplot(aes(ex,Bias_per_10F, fill = Methodology), width = .5) +
    facet_grid(vars(Analysis),vars(Simulated_RR), switch = "x") +
    ylab("Bias") +
    theme_minimal() +
    scale_x_continuous(breaks = NULL) +
    theme(legend.position = "bottom",
  ...
  return(bias_comp_plot)
}

coverage_comparison_visualization <- function(conditional_results,unconditional_results,number_of_repeats){
  ...
  overall_results <- rbind(conditional_results,unconditional_results) %>%
    ...
    mutate(Exp_ConfLow = exp(conf.low*10),
           Exp_ConfHigh = exp(conf.high*10),
           Exp_Estimate = exp(estimate*10)) %>%
    dplyr::select(Round_of_Sim, Exp_Estimate, Exp_ConfLow, Exp_ConfHigh, Simulated_RR, Analysis, Methodology)

  Coverage1 <<- overall_results %>%
    ungroup() %>%
    mutate(Covered = if_else(Simulated_RR>=Exp_ConfLow & Simulated_RR<=Exp_ConfHigh, 1, 0)) %>%
